...
const TARGETS = `[
  ...
  // ── speech / media devices (native VGC Core paths) ──
...
async function main(): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      for (const r of rows) {
        const label = r.v === 'ok' ? 'OK' : r.v
        if (r.v !== 'ok') console.log(`${label.padEnd(11)} ${r.t.padEnd(28)} ${r.s}`)
      }
      console.log(`\n${rows.filter((r) => r.v === 'ok').length} native-ok; ${bad.filter((b) => b.v === 'ANON').length} ANON; ${bad.filter((b) => b.v === 'SOURCE-LEAK').length} SOURCE-LEAK`)
      const leaks = bad.filter((b) => b.v === 'SOURCE-LEAK')
      const anon = bad.filter((b) => b.v === 'ANON')
      console.log(leaks.length ? 'FAIL: JavaScript source is visible' : anon.length ? 'FAIL: anonymous native names detected' : 'PASS: all fingerprint functions/getters are native and named')
