...
> Apply order on a fresh `out/vgc` checkout: `git apply engine-src/patches/01-*.patch engine-src/patches/02-*.patch engine-src/patches/06-*.patch engine-src/patches/07-*.patch engine-src/patches/08-*.patch` (plus `vgc-uach-chrome-brand.patch`), copy `AppIcon.icon` over `chrome/app/theme/chromium/mac/AppIcon.icon`, rebuild `chrome`, then `scripts/package-mac-engine.sh`. Windows: same `git pull` + apply 01/02/06/07/08 + rebuild its engine + build the .exe to get native antidetect there.
