...
- **`02-forward-vgc-switches.patch`** ✅ — `ChromeContentBrowserClient::AppendExtraCommandLineSwitches` forwards the `--vgc-*` switches (incl. `vgc-screen`, `vgc-color-depth`, `vgc-fonts`, `vgc-seed`) to **child (renderer) processes**. REQUIRED: WebGL/screen/client-rects/fonts all run in the renderer; without forwarding the renderer never sees the switch and returns the real value.
...
- **`07-client-rects-native.patch`** ✅ VERIFIED — `Element::getBoundingClientRect` / `getClientRects` farble the rect natively (per-profile `--vgc-seed`). Built + validated on M151: farbled from a NATIVE getter, no JS override. The JS layer auto-detects it (a detached element rect is non-zero) via `__vgcRectsNative` and skips its own spoof — so no double-noise and no JS getter tell. Real git-apply-able diff.
- **`08-fonts-native.patch`** ✅ VERIFIED — `FontCache::GetFontPlatformData` returns `nullptr` for any `kCreateFontByFamily` family NOT in the per-profile `--vgc-fonts` allowlist → the font falls back and enumerates as "not installed". Built + validated on M151: allowlisted fonts (e.g. Georgia, Helvetica Neue) measure their true width; real Mac fonts OUTSIDE the list (Papyrus, Chalkboard, Zapfino) measure the fallback width = hidden — via the actual `measureText`/`offsetWidth` fingerprinting path. Generics (serif/sans-serif/monospace/system-ui/-apple-system), `emoji`/`symbol` families, and `.`-prefixed system UI fonts always pass (rendering never breaks; empty `--vgc-fonts` disables the filter). `document.fonts.check()` is unaffected — it returns `true` for ANY family name even in STOCK Chrome (verified: a made-up font checks `true`), so it is not a detection vector and matching stock behavior avoids a tell. `vgc-fonts` added to kVgcSwitches (02). Real git-apply-able diff.
- **`09-jsheap-native.patch`** ✅ VERIFIED — `MemoryInfo`/`GetHeapSize` (`memory_info.cc`) pins `performance.memory.jsHeapSizeLimit` to the canonical value for the CLAIMED RAM class (`--vgc-device-memory`: ≤2→1090519040, ≤4→2172649472, else 4395630592), capping DOWN only (never claims more heap than the machine has). Real Chrome scales this with PHYSICAL RAM, so a profile spoofing deviceMemory:4 on a big rig otherwise exposed a ~4GB heap limit — a creepjs/pixelscan contradiction. Native ⇒ the getter reports native WITH name (the JS override left an anonymous-name toString tell). Reuses `vgc-device-memory` (already in kVgcSwitches).
- **`10-mediadevices-label-native.patch`** ✅ VERIFIED — `MediaDeviceInfo::label()` (`media_device_info.cc`) returns `""` when `--vgc-seed` is set. The mic/cam model string ("HD Pro Webcam C920", …) is identical across every profile on a host = a same-machine correlator; deviceId/groupId are already per-profile salted. `toJSONForBinding()` calls `label()`, so `JSON.stringify(device)` is covered too. Native ⇒ no JS prototype override (which left an anonymous-name tell on label + toJSON).
- **`11-matchmedia-coherence-native.patch`** ✅ VERIFIED — `MediaValues::CalculateDeviceWidth/Height` (`media_values.cc`) return the spoofed `--vgc-screen=WxH` so `@media (device-width: <spoofed>px)` and `matchMedia('(max-device-width: …)')` stay coherent with `screen.width` (screen.cc). This is what let 2.1.63 DELETE the JS matchMedia Proxy shim that broke Cloudflare Turnstile 600010 — the coherence is now native (undetectable) and no wrapper touches `window.matchMedia`. Reuses `vgc-screen`.
- **`12-speech-voices-native.patch`** ✅ VERIFIED — `SpeechSynthesis::OnSetVoiceList` (`speech_synthesis.cc`) keeps a deterministic per-`--vgc-seed` ~75% subset of the OS TTS voice list (index 0 always kept). The full voice list is identical across every profile on a machine (correlator) and leaks the host OS voice set; the subset makes two profiles enumerate DIFFERENT lists, stable across `getVoices()` calls. Native ⇒ `getVoices` reports native WITH name (the JS override left an anonymous-name tell). Reuses `vgc-seed`.
...
> Apply order on a fresh `out/vgc` checkout: `git apply engine-src/patches/01-*.patch engine-src/patches/02-*.patch engine-src/patches/06-*.patch engine-src/patches/07-*.patch engine-src/patches/08-*.patch engine-src/patches/09-*.patch engine-src/patches/10-*.patch engine-src/patches/11-*.patch engine-src/patches/12-*.patch` (plus `vgc-uach-chrome-brand.patch`), copy `AppIcon.icon` over `chrome/app/theme/chromium/mac/AppIcon.icon`, rebuild `chrome`, then `scripts/package-mac-engine.sh`. Windows: `scripts\build-win-engine.bat` applies the combined `vgc-native-all.patch` (which already contains 06–12) + `vgc-uach-chrome-brand.patch` and rebuilds. All native vectors verified undetectable: verify:tostring 0 anon / 0 source-leak, verify:deep 0 leak / 0 tamper / 0 coherence, verify:correlation no high-signal correlator.
...
| **Fonts** | `third_party/blink/renderer/platform/fonts/font_cache.cc` / platform font enumeration | font availability lookup | ✅ `08-fonts-native.patch` — `GetFontPlatformData` returns nullptr for families outside the per-profile `--vgc-fonts` allowlist (generics/emoji/system UI always pass). Blocks the `measureText`/`offsetWidth` fingerprinting path; `document.fonts.check()` matches stock (returns true for any name). |
